...
def run_linear(
    ...
    # Set self.W to starting weights
    state_dict = {
        'W': weights
    }
...
def run_embedding(
    ...
    """
    embedding_layer = embedding.Embedding(num_embeddings=vocab_size, embeddings_dim=d_model)
    # Set self.W to starting weights
    state_dict = {
        'W': weights
    }
...
def run_swiglu(
    ...
    # Set self.W to starting weights
    state_dict = {
        'W1.W': w1_weight,  # The W in linear is [d_out, d_in]
        'W2.W': w2_weight,
        'W3.W': w3_weight,
    }
...
def run_multihead_self_attention(
    d_model: int,
    num_heads: int,
    q_proj_weight: Float[Tensor, " d_k d_in"],  # This note is wrong. q_proj_weight = [d_model, d_model]
    k_proj_weight: Float[Tensor, " d_k d_in"],  # This note is wrong. k_proj_weight = [d_model, d_model]
    v_proj_weight: Float[Tensor, " d_v d_in"],  # This note is wrong. v_proj_weight = [d_model, d_model]
    ...
    Args:
        ...
        q_proj_weight (Float[Tensor, "d_k d_in"]): Weights for the Q projection -> this note is wrong. q_proj_weight = [d_model, d_model]
        k_proj_weight (Float[Tensor, "d_k d_in"]): Weights for the K projection -> this note is wrong. k_proj_weight = [d_model, d_model]
        v_proj_weight (Float[Tensor, "d_k d_in"]): Weights for the V projection -> this note is wrong. v_proj_weight = [d_model, d_model]
    ...
    """
    multihead_self_attention = attention.CausalMultiHeadSelfAttention(d_model, num_heads)
    multihead_self_attention.from_weights(q_proj_weight, k_proj_weight, v_proj_weight, o_proj_weight)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    rope_config = rope.RopeConfig(theta=theta, d_k=int(d_model / num_heads), max_seq_len=max_seq_len)
    multihead_self_attention = attention.CausalMultiHeadSelfAttention(d_model, num_heads, rope_config)
    multihead_self_attention.from_weights(q_proj_weight, k_proj_weight, v_proj_weight, o_proj_weight)
    return multihead_self_attention.forward(in_features)
...
def run_transformer_block(
    ...
    """
    rope_config = rope.RopeConfig(theta=theta, max_seq_len=max_seq_len, d_k=int(d_model / num_heads))
    ...
    transformer_block.from_weights(weights)
...
def run_transformer_lm(
    ...
    """
    trans = transformer_lib.Transformer(
        vocab_size,
        context_length,
        d_model,
        num_layers,
    ...
    trans.from_weights(weights)
    return trans.forward(in_indices)
...
def run_rmsnorm(
    ...
    """
    rmsnorm_layer = rmsnorm.RMSNorm(d_model=d_model, eps=eps)

    # Set self.g to starting weights
    state_dict = {
        'g': weights
    }
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    """
    result = softmax.softmax(in_features, dim)
    # result = softmax.softmax(result, 0)
    # result = softmax.softmax(result, 1)
    return result
...
def run_cross_entropy(
    ...
    """
    return loss_function.avg_cross_entropy(inputs, targets)
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    return gradient_clipping.clip_gradients(
...
def run_get_lr_cosine_schedule(
    ...
    """
    return learning_rate_schedule.lr_cosine_schedule(
        it=it,
...
def run_save_checkpoint(
    ...
    """
    return checkpointing.save_checkpoint(
        model=model,
        optimizer=optimizer,
        iteration=iteration,
        out=out
...
def get_tokenizer(
    ...
    """
    return bpe_encoding.Tokenizer(vocab, merges, special_tokens)
